...
public class CodeWriter extends DexCodeVisitor {
    ...
    public int in_reg_size = 0;
    public int max_out_reg_size = 0;
    public List<Insn> ops = new ArrayList<>();
    public List<Insn> tailOps = new ArrayList<>();
    int total_reg;
	...
	public void visitPackedSwitchStmt(Op op, int aA, final int first_case, final DexLabel[] labels) {
		...
		Label[] transformLabels = new Label[labels.length];
		for (int i = 0; i < labels.length; ++i) {
			transformLabels[i] = getLabel(labels[i]);
		}
		tailOps.add(new PackedSwitch(jumpOp, first_case, transformLabels));
	}
	...
	public void visitSparseSwitchStmt(Op op, int ra, final int[] cases, final DexLabel[] labels) {
		...
		tailOps.add(new SparseSwitch(jumpOp, cases, transformLabels));
	}
    ...
    public static class OP3rc extends OpInsn {
        ...
        public OP3rc(Op op, int start, int length, BaseItem item) {
			super(op);
			this.start = start;
			this.length = length;
			this.item = item;
	...
	public static class PackedSwitch extends Insn {
		public Label[] labels;
		public int first_case;
		public JumpOp jumpOp;

		public PackedSwitch(JumpOp jumpOp, int first_case, Label[] labels) {
			this.jumpOp = jumpOp;
		...
		public void write(ByteBuffer out) {
			...
			for (int i = 0; i < labels.length; i++) {
				out.putInt(labels[i].offset - jumpOp.offset);
			}
		}
	}
    
	public static class SparseSwitch extends Insn {
		public Label[] labels;
		public int[] cases;
		public JumpOp jumpOp;

		public SparseSwitch(JumpOp jumpOp, int[] cases, Label[] labels) {
			this.jumpOp = jumpOp;
			this.cases = cases;
			this.labels = labels;
		}
