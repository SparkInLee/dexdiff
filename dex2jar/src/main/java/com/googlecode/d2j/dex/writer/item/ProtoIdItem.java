...
public class ProtoIdItem extends BaseItem implements Comparable<ProtoIdItem> {
	...
	public String toString() {
		...
		sb.append("(");
		if (null != parameters) {
			List<TypeIdItem> items = parameters.items;
			if (null != items && items.size() > 0) {
				for (TypeIdItem item : items) {
					sb.append(item.descriptor.stringData.string);
				}
			}
		}
		sb.append(")").append(ret.descriptor.stringData.string);
		return sb.toString();
	}
}